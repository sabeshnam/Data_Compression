/*                                      HUFFMAN CODING         
*************************************************************************************        
 ALGORITHM :- 	Uses a priority queue. Using it, it makes a prefix tree(code of any 
		character cannot be the prefix of any other characeter else the 
		decomposition wont work) based on priorities of various characters. 
		Letters with higher priority are towards the top while letters with 
		lower priority are made the leaves and hence have longer encoding 
		than those with higher priority.                        

 CONCEPT :- 	each ASCII character is usually represented with 8 bits, but 
		if we had a text filed composed of only the lowercase a-z letters 
		we could represent each character with atmost 5 bits (i.e., 2^5 = 32, 
		which is enough to represent 26 values), thus reducing the overall 
...
void compressFile(FILE *input, FILE *output, int codeTable[])  /*function to compress the input*/
{
    	char bit, c, x = 0;
    	int n,length,bitsLeft = 8;
    	int originalBits = 0, compressedBits = 0;

    	while ((c=fgetc(input))!=10)                // checking for newline
	{
        	originalBits++;
        	if (c==32)
		{
    	...
    	if (bitsLeft!=8)
	{
    	...
    	fprintf(stderr,"Saved %.2f%% of memory\n",((float)(originalBits*8-compressedBits)/(originalBits*8))*100);
